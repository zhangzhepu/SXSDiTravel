...
<body> 
	...
	<div class="l_box f_l">
		...
		<div class="topnews">
			...
			<div >
				<ul>
					<font size="5"><a href="servlet/ShowDiaryAction?diaryId=${diary.diary.diaryId}">${diary.diary.diaryTitle}</a></font>
					<br/>
					<s:set name="content" value="diary.diary.diaryContent"/>
					<p>
					<%
					DiaryFull d=(DiaryFull) request.getSession().getAttribute("diary");
					Blob b = d.getDiary().getDiaryContent();
					String content = new String(b.getBytes((long)1, (int)b.length()));
					out.print(content);
					%>
					</p>
					<p class="autor">
						<span class="lm f_l"><a href="/">个人游记</a> 作者 ${diary.userName} </span><span
							class="dtime f_l">${diary.diary.diaryTime}</span><span
							class="viewnum f_r">浏览（<a href="/">${diary.diary.readTimes}</a>）
						</span>
